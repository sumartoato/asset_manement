...
export default async function ByLocationReportPage() {
  ...
  const rows = grouped
    .map((g) => ({
      ...
      name: g.locationId ? (locationMap.get(g.locationId) ?? "Tidak diketahui") : "Belum ditentukan",
      count: g._count._all,
  ...
  return (
    <div>
      <PageHeader title="Laporan per Lokasi" description="Jumlah dan total nilai aset dikelompokkan berdasarkan lokasi" />
      ...
      <Card>
        {rows.length === 0 ? (
          ...
          <Table>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.locationId ?? "none"} className="hover:bg-slate-50">
                  <Td>{r.name}</Td>
                  <Td className="text-right">{r.count}</Td>
                  <Td className="text-right">{formatCurrency(r.totalValue)}</Td>
                  <Td>
                    {r.locationId && (
                      <Link href={`/reports/asset-register?locationId=${r.locationId}`} className="text-indigo-600 hover:underline text-xs">
                        Lihat detail &rarr;
                      </Link>
                    )}
                  </Td>
